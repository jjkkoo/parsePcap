 ...
 *    Test result
 */
Word32 E_GAIN_clip_test(Word16 mode, Float32 mem[])
{
   Word32 clip;
   Float32 thres;
   ...
   if( (mode == 0) || (mode == 1) )
   {
	   thres = 0.9F + (0.1F * mem[0]/DIST_ISF_MAX_IO);	/* clipping is activated when filtered pitch gain > threshold (0.94 to 1) */
	   if (mem[1] > thres)
	   {
		   clip = 1;
	   }
   }
   else if ((mem[0] < DIST_ISF_THRES) && (mem[1] > GAIN_PIT_THRES))
   {
      clip = 1;
   }
 ...
 *    void
 */
void E_GAIN_clip_isf_test(Word16 mode, Float32 isf[], Float32 mem[])
{
   Word32 i;
   ...
   if( (mode == 0) || (mode == 1) )
   {
	   if (dist > DIST_ISF_MAX_IO)
	   {
		   dist = DIST_ISF_MAX_IO;
	   }
   }
   else if (dist > DIST_ISF_MAX)
   {
	   dist = DIST_ISF_MAX;
   }
 ...
 *    void
 */
void E_GAIN_clip_pit_test(Word16 mode, Float32 gain_pit, Float32 mem[])
{
   Float32 gain;

   if( (mode == 0) || (mode == 1) )
   {
	   gain = 0.98F*mem[1] + 0.02F*gain_pit;	  /* long term LTP gain average (>250ms) */
   }
   else
   {
